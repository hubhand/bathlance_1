...
export const EditProductScreen: React.FC<EditProductScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const monthsToAdd =
      periodAfterOpening || AVERAGE_USAGE_PERIODS[category] || 6;
  ...
  const handleAnalyzeIngredients = async () => {
    ...
    try {
      ...
      // 에러 메시지 처리
      let errorMessage =
        "성분 분석에 실패했어요. 이미지에 전성분표가 잘 보이는지 확인 후 다시 시도해주세요.";
  ...
  return (
    <div className="p-2 sm:p-4 max-w-full overflow-x-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
        ...
        <div className="pt-2">
          <h3 className="text-md font-bold text-gray-600 mb-2 flex items-center">
            AI 성분 분석 🔬{" "}
            <span className="text-xs font-normal text-gray-500 ml-2">
              (제품 뒷면 사진 필요)
            </span>
          </h3>
          {formData.ingredientAnalysis &&
          formData.ingredientAnalysis.length > 0 ? (
        ...
        <div className="flex items-center justify-between bg-red-50 border-l-4 border-red-400 p-4 rounded-r-lg">
          <div className="flex-1">
            <label htmlFor="hasTrouble" className="font-bold text-red-700">
              피부 트러블 발생! 😥
            </label>
        ...
        <div className="flex space-x-2 sm:space-x-4 pt-3 sm:pt-4">
          <button
            type="button"
            onClick={onCancel}
            className="w-1/2 bg-gray-300 text-gray-800 font-bold py-2 sm:py-3 px-3 sm:px-4 rounded-full shadow-md hover:bg-gray-400 transition-colors text-sm sm:text-base"
          >
            취소
          </button>
          <button
            type="submit"
            className="w-1/2 bg-bathlance-orange text-white font-bold py-2 sm:py-3 px-3 sm:px-4 rounded-full shadow-lg hover:bg-orange-600 transition-colors text-sm sm:text-base"
          >
            저장하기
          </button>
        </div>
